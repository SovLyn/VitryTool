...
> **当前状态**：`0.3.0`，剪贴板历史（含收藏）+ 快速粘贴（全局快捷键小屏）+ 局域网剪贴板同步（lan-sync，libp2p + mDNS）+ **局域网文件共享（lan-file，点对点加密传输 + 自动图片通道）** + **移动端（Android）支持**（收件箱接收 → 写剪贴板），含日志系统、主题、设置页、全局通知（toast）、托盘（菜单文案随语言切换、lan-sync 广播/接收与文件共享快速开关）与窗口状态记忆。

## 项目简介

VitryTool 的目标是提供轻量、本地的局域网信息共享能力，不依赖云端服务，所有数据在局域网内流转。

当前仓库为**框架骨架 + 五个功能**：前后端分离架构、接口契约流程、文档体系与工程规范已就位，具体功能在 `docs/features/` 中规划推进。
...
- **已落地**：
  ...
  - **局域网文件共享（lan-file，0.3.0）**——桌面终端间点对点推送任意文件：选文件 → 点终端 → 对端确认（首次 TOFU）→ 独立加密 TCP 流式传输（断线自动续传、取消即永久终止、全文件哈希对账、原子落盘）；并打通**自动图片通道**——信任终端复制截图，收件箱条目自动点亮为真实图片。发现/信令复用 lan-sync 节点（gossipsub 公告），见 [`docs/features/lan-file.md`](docs/features/lan-file.md) 与接口契约 [`docs/api/lan-file.md`](docs/api/lan-file.md)。
...
- **剪贴板历史性能**：历史列表通过 tauri-plugin-store 整体序列化持久化，接近上限（1024 条富文本）时**初次全量加载与上限截断**仍有序列化开销；每次复制的增量更新与列表渲染已优化（见 CHANGELOG Unreleased）。
...
- **局域网同步（lan-sync，0.2.5）已知限制**（调研与实测结论）：
...
- **移动端（Android，0.2.9）已知限制**（详见 [`docs/features/mobile.md`](docs/features/mobile.md)）：
  ...
  - **仅文本写入**：图片/文件字节无法写入手机剪贴板（图片仅元数据占位文本、文件路径条目禁用写回；0.3.0 起手机可**接收**自动送达的图片字节到收件箱——门控为「noise 已认证身份 + 该终端 5 分钟内在剪贴板主题活跃过」，无人工信任 UI——但写剪贴板仍是文本占位）。
...
- **局域网文件共享（lan-file，0.3.0）已知限制**（详见 [`docs/features/lan-file.md`](docs/features/lan-file.md)）：
  ...
  - **链路差时传输会变慢并自动重连续传**：WiFi 丢包会让 TCP 拥塞窗口塌缩（实测吞吐可掉到几十 KB/s，观感像「卡住」）。单块 30 秒零进展即判定链路已死 → 自动重连并从断点续传（接收侧 60 秒无数据同理放行会话）；期间传输卡显示「网络中断，等待恢复…」。有线链路体验明显更好。
  - **不支持文件夹**：拖入文件夹会在入列前被拦下并提示（v1 只传单个文件，无目录递归）。
  - 单会话（同时仅一个传输任务，忙时新提议自动拒绝）；断点续传仅对异常中断开放（取消=永久终止）；无跨重启传输历史；移动端仅接收图片通道产物且需前台。

## 版本管理

当前版本 `0.3.0`。每次有新功能签发时按约定递增版本，规则见 [`docs/versioning.md`](docs/versioning.md)。
